...
if (!empty($category_slug)) {
    ...
    $cat_query = "SELECT id, name FROM blog_categories WHERE slug = :slug LIMIT 1";
    ...
    if ($category) {
        ...
        // Join with post_categories table
        $where_clause = " WHERE p.id IN (SELECT post_id FROM blog_post_categories WHERE category_id = :category_id) ";
...
if (empty($where_clause)) {
    ...
    $where_clause .= " AND p.status = 'published' ";
}
...
// Set page variables
$page_title = !empty($category_name) ? $category_name . ' - Blog' : 'Blog';
...
<section class="hero-section text-center">
    <div class="container">
        <h1><?php echo htmlspecialchars(!empty($category_name) ? $category_name : 'Our Blog'); ?></h1>
        <p class="lead">
            <?php echo htmlspecialchars(!empty($category_name) 
                ? 'Browse our latest posts in ' . $category_name
                : 'Latest news, tips, and insights about sports cards and collectibles'); ?>
        </p>
    </div>
</section>
        ...
        <div class="col-lg-8">
            <?php if (empty($posts)): ?>
                <div class="alert alert-info">
                    ...
                    <p>There are currently no blog posts in this category. Please check back later or browse other categories.</p>
                    ...
                    <nav aria-label="Blog pagination" class="blog-pagination mt-4">
                        <ul class="pagination justify-content-center">
                            <?php if ($current_page > 1): ?>
                                <li class="page-item">
                                    <a class="page-link" href="?page=<?php echo $current_page - 1; ?><?php echo !empty($category_slug) ? '&category=' . htmlspecialchars($category_slug) : ''; ?>">
                            ...
                            if ($start_page > 1) {
                                echo '<li class="page-item"><a class="page-link" href="?page=1' . (!empty($category_slug) ? '&category=' . htmlspecialchars($category_slug) : '') . '">1</a></li>';
                            ...
                            ?>
                                <li class="page-item <?php echo $i == $current_page ? 'active' : ''; ?>">
                                    <a class="page-link" href="?page=<?php echo $i; ?><?php echo !empty($category_slug) ? '&category=' . htmlspecialchars($category_slug) : ''; ?>">
                                        <?php echo $i; ?>
                                    </a>
                                </li>
                            <?php 
                            ...
                            if ($end_page < $total_pages) {
                                ...
                                echo '<li class="page-item"><a class="page-link" href="?page=' . $total_pages . (!empty($category_slug) ? '&category=' . htmlspecialchars($category_slug) : '') . '">' . $total_pages . '</a></li>';
                            }
                            ?>
